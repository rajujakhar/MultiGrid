...
MultiGridSolver::MultiGridSolver(const size_t &numLevel, const size_t &numVcycle, const TwoDimArr &u, bool isNeumannBC ):
    numLevel_(numLevel), numVcycle_(numVcycle), neu1_(2), neu2_(1),currVCycle_(1),isNeumannBC_(isNeumannBC)
{
...
//                refreshBoundaries(level,isNeumannBC_);
...
void MultiGridSolver::applyRBGS_Iter(const size_t& level)
{
      ...
      // #pragma omp parallel for schedule(static)
       ...
       //#pragma omp parallel for schedule(static)
        ...
        // Update boundary vertical walls to satisfy Neumann BCs  
        if(isNeumannBC_)
        refreshBoundaries(level);
	 
       
        
}
// Updating the Ghost layer with the previous iteration values
void MultiGridSolver::refreshBoundaries(const size_t& level)
{
    const int cgInd = numLevel_- level;
    size_t numGrid = gridVec_[cgInd]->numGrid_;
    size_t loopMax = numGrid-1;
//    const real h = gridVec_[cgInd]->h_;

    assert(cgInd>=0);

        if(cgInd ==0)                   // Finest level,  update the boundary values as per Neumann logic 
        {

            for (size_t i=1; i<loopMax; ++i) // For copying values to left & right Ghost boundary layer
            {
                //Updating Ghost layer values in order to satisfy Neuman BC which is used in next iteration
                gridVec_[cgInd]->u_(i,0,numGrid)=   gridVec_[cgInd]->u_(i,1,numGrid) - gridVec_[cgInd]->h_  ;
                gridVec_[cgInd]->u_(i,loopMax,numGrid)= gridVec_[cgInd]->u_(i,loopMax-1,numGrid) - gridVec_[cgInd]->h_ ;
        ...
        else
        {
            for (size_t i=1; i<loopMax; ++i)     // For copying values to left & right Ghost boundary layer
            {
                gridVec_[cgInd]->u_(i,0,numGrid)=   gridVec_[cgInd]->u_(i,1,numGrid) ;
                gridVec_[cgInd]->u_(i,loopMax,numGrid)= gridVec_[cgInd]->u_(i,loopMax-1,numGrid) ;
        ...
        //displayGrid(level,currVCycle_, std::string("u")); 
        //std::cout << "Neumann called at level "  << level << " v cycle " << currVCycle_ << std::endl;
