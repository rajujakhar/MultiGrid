...
MultiGridSolver::MultiGridSolver(const size_t &numLevel, const size_t &numVcycle, const TwoDimArr &u ): numLevel_(numLevel), numVcycle_(numVcycle), neu1_(1), neu2_(1),currVCycle_(1)
{
...
{
        real resNormNew, resNormOld=0, convRate, dummy;
        ...
        for(size_t count=0; count< numVcycle_; ++count)
	{
	       ...
	       std::cout << "Discrete L2 residum norm after " <<  count+1 << " V-Cycle is " << resNormNew << std::endl;	
	        ...
	        //displayGrid(numLevel_, count+1,std::string("res"));
	        
	        if(count >0)
	        {
	                convRate = resNormNew/resNormOld;
                       // std::cout << "Convergence rate after " << count+1 << " V-Cycle    ----------->    " << convRate << std::endl;	        
	        }
	        
	        // Display the current residual after each v cycle
	       
	        resNormOld = resNormNew;
	        
	        // Print u after 1 v cycle 
	        //applyRBGS_Iter(numLevel_);
	        //displayGrid(numLevel_,currVCycle_, std::string("u"));  
        ...
        dummy = convRate;
        std::cout << "wahe guru  "<<dummy << std::endl;
...
{        
       ...
       const real numInnerPoints = (numGrid-2)*(numGrid-2);
        //const real numInnerPoints = (numGrid-0)*(numGrid-0);
...
void MultiGridSolver::applyInterpolation(const size_t& level)
{
        ...
        // This code is copied from sachin's project to check for the residual error
       /* for(size_t i=1; i<numGrid-1; ++i)
        {
                for(size_t j=1; j<numGrid-1; ++j)
                {
                
                gridVec_[cgInd-1]->u_(2*i-1,2*j-1,numGridFiner)	+= 	p_.sw_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i-1,2*j,numGridFiner)	+= 	p_.s_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i-1,2*j+1,numGridFiner)  	+= 	p_.se_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i,2*j-1,numGridFiner) 	+= 	p_.w_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i,2*j,numGridFiner)         	+= 	p_.c_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i,2*j+1,numGridFiner) 	+= 	p_.e_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i+1,2*j-1,numGridFiner) 	+=    	p_.nw_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i+1,2*j,numGridFiner) 	+= 	p_.n_*gridVec_[cgInd]->u_(i,j,numGrid);
		gridVec_[cgInd-1]->u_(2*i+1,2*j+1,numGridFiner) 	+= 	p_.ne_*gridVec_[cgInd]->u_(i,j,numGrid);
...
void MultiGridSolver::mgmSolve(const size_t& level)
{
       ...
       // }                              // Its time to go to the coaser level :)
        ...
        if(level == 1)
                ...
                {      
                        //std::cout << "Print of u before solving for exact solution\n " << std::endl; 
                        //displayGrid(level,currVCycle_, std::string("u"));   
...
void MultiGridSolver::applyRBGS_Iter(const size_t& level)
{
      ...
      /*  for(size_t i=1;i<numGrid-1;++i){
		...
		if(i & 1)
		{
            for(size_t j=1;j<numGrid-1;j+=2){

			gridVec_[cgInd]->u_(i,j,numGrid) = 0.25*(gridVec_[cgInd]->u_(i,j-1,numGrid)	+
								gridVec_[cgInd]->u_(i,j+1,numGrid)	+
								gridVec_[cgInd]->u_(i-1,j,numGrid)	+
								gridVec_[cgInd]->u_(i+1,j,numGrid)	+
                                (hSq * gridVec_[cgInd]->f_(i,j,numGrid)));
		}
	    }
		else{

			for(size_t j=2;j<numGrid-1;j+=2){
