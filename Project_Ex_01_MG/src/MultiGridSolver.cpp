...
MultiGridSolver::MultiGridSolver(const size_t &numLevel, const size_t &numVcycle, const TwoDimArr &u ): numLevel_(numLevel), numVcycle_(numVcycle), neu1_(2), neu2_(1)
{
      ...
      //  std::cout << this->gridVec_[0];
       ...
       this->s_.setStencilParams(4, -1, -1, -1);
...
void MultiGridSolver::mgmSolve(const size_t& level)
{
        ...
        if(level == 1)                   // This is the end of recursion
      ...
      // #pragma omp parallel for schedule(static)
        for(size_t i=1; i< loopMax; ++i)
        {
             ...
             j= ((isFirstCol ==true)?  1:2);
                ...
                while(j < loopMax)
                        {  
                                gridVec_[cgInd]->u_(i,j,numGrid) = 0.25*( hSq*(gridVec_[cgInd]->f_(i,j,numGrid)) +  gridVec_[cgInd]->u_(i+1,j,numGrid) + 
                                gridVec_[cgInd]->u_(i-1,j,numGrid) + gridVec_[cgInd]->u_(i,j+1,numGrid) + gridVec_[cgInd]->u_(i,j-1,numGrid) ) ;
